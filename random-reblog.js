...
function init(){
  ...
  function GetPostKey(){
    ...
    if (tagList.length < 5000) { // ex 5000 - Size limit of the taglist
      ...
      client.taggedPosts(tag, { filter: 'html' }, function (err, data) {
        if (data != undefined) {
  ...
  function reblogMachine(){
    ...
    var myCom = 'A simple comment'; // comment: if you want to add a comment on every rebloged-post. Leave empty if you want.
      if (key != undefined && postIdList.length != 0 ){
        client.reblogPost(blogName, { id: postId, reblog_key: key, comment: myCom}, function (err, data) {
          if (err) {
                console.log(throw new error);
              } else {
                console.log('NOTIF PUSH OKAY'+response);
              }
            });
            // ----------------------------- ERROR LOG -------------------------
  ...
  setInterval(reblogMachine, 360000);// time for reblog 1000 = 1s (max 250 reblog/day)
}
init();
